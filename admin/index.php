...
// Revenus du mois
 $revenusMois = $db->query("SELECT COALESCE(SUM(montant), 0) as total FROM Paiements 
    WHERE MONTH(date_paiement) = MONTH(CURRENT_DATE()) AND recu = TRUE")->fetchColumn();
...
<div class="bg-slate-100 min-h-screen p-6 md:p-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Bienvenue, <?= htmlspecialchars($_SESSION['user_name'] ?? 'Invité') ?></p>
    </div>
